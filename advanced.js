...
bindSquares();
document.querySelectorAll('.opening-card').forEach(card => card.onclick = () => { const o = openingLibrary[card.dataset.opening]; document.querySelector('.turn-banner b').textContent = `${o.name} · Main line`; document.querySelector('.turn-banner span').textContent = `STUDYING ${o.eco}`; document.querySelector('.coach-card p').innerHTML = `<b>${o.name}</b> ${o.note}`; document.querySelector('.study-tabs button[data-study="board"]').click(); showAnalysis(); });
